; (function () {
  ...
  checkbox.addEventListener("change", function () {
    ...
    var newval = curval.replace(/ *\[[^)]*\] */g, "");
    input.value = newval;

    if (checkbox.checked) {
      //remove/add
      dsvg.setAttribute("fill", "#F04263");
      input.value += str;
    } else {
      dsvg.setAttribute("fill", "#6F6F6F");
    }
  ...
  if (searchFormContainer) {
    searchFormContainer.addEventListener("submit", function () {
      return true;
    }, false);
  }
  ...
  interval = setInterval(function() {
    ...
    targetNode1 = document.querySelector('#contents.style-scope.ytd-item-section-renderer .ytd-watch-next-secondary-results-renderer');
    if (targetNode1 && !observingTargetNode1) {
      observingTargetNode1 = true;
      observeDOM(targetNode1, function (m) {
        waitForVideoResults();
      });
    }
